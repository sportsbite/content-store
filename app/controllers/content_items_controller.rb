...
private
  ...
  def content_id_header
    request.headers['Govuk-Content-Id']
  end

  def can_view(item)
    if content_id_header.present?
      content_id_header == item.content_id
    else
  ...
  def set_cache_headers(item, intent)
    ...
    elsif item && (content_id_header.present? || item.access_limited?)
